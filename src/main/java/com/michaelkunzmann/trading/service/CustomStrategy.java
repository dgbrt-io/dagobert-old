...
//		if (lastPeriod == null) {
//			logger.info("Warte bis aktuelle Periode zu Ende ist... noch "
//					+ ((currentPeriod.getToTime().getTime() - new Date()
//							.getTime()) / 1000L) + " Sekunden");
//			return null;
//		}
		...
		if (enoughBtcToSell) {
			...
			double sellPrice = statistics
					.getAdvancedStatistics(defaultCurrency).getSell()
					.getValue()
					- GLUECKSKREUZER;

			// Current rate rater higher than buy price + buy fee + sell fee?
			if (sellPrice > buyTransaction.getRate().getValue()
					* (1.0 + feePerc) * (1.0 + MIN_ROI_PER_TRADE)
					/ (1.0 - feePerc)) {
				Order order = new Order();
				order.setAmount(new CurrencyData(balanceBtc.getValue(),
						CurrencyType.BTC));
				order.setPrice(new CurrencyData(sellPrice, defaultCurrency));
				order.setType(OrderType.ASK);
				order.setCurrency(defaultCurrency);

				return order;
			} else {
				/*logger.info("Aktueller Kurs müsste zum verkauf über "
						+ buyTransaction.getRate().getValue() * (1.0 + feePerc)
						* (1.0 + MIN_ROI_PER_TRADE) / (1.0 - feePerc)
						+ " liegen. (Ist: " + sellPrice + ")");*/
			}
		}
		...
		if (enoughMoneyToBuy) {
			...
			double buyPrice = statistics.getAdvancedStatistics(defaultCurrency)
					.getBuy().getValue()
					+ GLUECKSKREUZER;

			boolean beyondLastSellPriceAndGrowingRate = buyPrice < sellTransaction
					.getRate().getValue() / (1.0 + MIN_GROWTH_RATE);
			...
			if (beyondLastSellPriceAndGrowingRate) {
				...
				order.setPrice(new CurrencyData(buyPrice, defaultCurrency));
				order.setType(OrderType.BID);
				order.setCurrency(defaultCurrency);
				return order;
			} else {
//				logger.info("Aktueller Kurs müsste zum kaufen unter "
//						+ sellTransaction.getRate().getValue()
//						/ (1.0 + MIN_GROWTH_RATE) + " liegen. (Ist: "
//						+ buyPrice + ")");
			}
		}
